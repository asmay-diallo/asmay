...
//   };
// };
// useIncomingCall.ts - VERSION SIMPLIFIÉE
...
export const useIncomingCall = () => {
  ...
  // Navigation automatique vers l'écran d'appel quand accepté
  // useEffect(() => {
  //   if (isCallAccepted && callData) {
  //     console.log('🧭 Navigation automatique vers écran d\'appel');
  //     router.push({
  ...
  //     });
  //   }
  // }, [isCallAccepted, callData?.callId]);
  // useIncomingCall.ts
// useEffect(() => {
//   if (isCallAccepted && callData) {
//     // // ✅ Vérifier si c'est le DESTINATAIRE qui doit naviguer
//     // const isReceiver = callData.calleeId === user?._id;
//     
//       console.log('🧭 [DESTINATAIRE] Navigation vers écran d\'appel');
//       router.push({
//         pathname: '/(main)/(asmay)/call',
//         params: {
//           callId: callData.callId,
...
//         },
//       });
//       console.log('📞 [APPELANT] Appel accepté,  navigation');
//       // L'appelant reste sur son écran, l'UI d'appel peut s'afficher autrement
//   }
// }, [isCallAccepted, callData?.callId]);
// useIncomingCall.ts
useEffect(() => {
  if (isCallAccepted && callData) {
    ...
    const isReceiver = callData.calleeId === userId;
    
    console.log('🧭 Navigation décision:');
    console.log('   userId:', userId);
    console.log('   calleeId:', callData.calleeId);
    console.log('   isReceiver:', isReceiver);
    
    // ✅ SEUL le destinataire navigue
    if (isReceiver) {
      // console.log('🧭 [DESTINATAIRE] Navigation vers écran d\'appel');
      // router.push({
      //   pathname: '/(main)/(asmay)/call',
      ...
      console.log('📞 [APPELANT] Pas de navigation');
      // ✅ L'appelant reste où il est
    }
  }
}, [isCallAccepted, callData?.callId, user?._id]);
  return { callState };
};